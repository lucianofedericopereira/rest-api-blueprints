...
**Dependency remediation — npm, Go, CodeQL (A.12 Operations security / A.14 System acquisition)**
- `iso27001-nestjs`: `npm audit fix` resolved all 9 flagged advisories, all satisfied by existing `package.json` semver ranges (no manifest changes needed) — `multer` 2.1.1→2.2.0 (DoS via deeply nested field names, GHSA-72gw-mp4g-v24j; incomplete cleanup of aborted uploads, GHSA-3p4h-7m6x-2hcm), `@nestjs/platform-express`→11.1.29 (pulled the fixed multer), `body-parser` 2.2.2→2.3.0 (invalid `limit` silently disabling size enforcement, GHSA-v422-hmwv-36x6), `brace-expansion`→1.1.18/2.1.4/5.0.9 across its three copies in the tree (exponential-time and unbounded-length DoS, GHSA-3jxr-9vmj-r5cp / GHSA-mh99-v99m-4gvg / GHSA-rgw5-rvv9-x895), `js-yaml`→4.3.1/3.15.1 (quadratic-CPU merge-key DoS and the un-backported `!!omap` CVE-2026-59870 fix, GHSA-h67p-54hq-rp68 / GHSA-52cp-r559-cp3m / GHSA-5p4m-2wfm-xmqj), `fast-uri`→3.1.5 (host confusion via backslash authority delimiter/introducer and failed IDN canonicalization, GHSA-v2hh-gcrm-f6hx / GHSA-7p8r-x3mc-p8w7 / GHSA-4c8g-83qw-93j6), `typeorm`→0.3.31 (`migration:generate` template-literal code injection, GHSA-2rp8-mm9q-fp49), `@babel/core`→7.29.7 (arbitrary file read via `sourceMappingURL`, GHSA-4x5r-pxfx-6jf8), and `ajv` (dev-only `schema-utils` chain)→8.18.0/6.15.0 (ReDoS via `$data`, GHSA-2g4f-4pwh-qvx6). `npm audit` reports 0 vulnerabilities; `jest` (39/39) and `nest build` still pass
- `iso27001-gin/go.mod`: `golang.org/x/crypto` 0.45.0→0.55.0, closing the batch of `x/crypto/ssh` advisories GitHub flagged (unenforced revoked-key status, agent key-constraint/forwarding-constraint bypass, FIDO/U2F presence-check bypass, pathological-input panics, deadlock and memory-leak DoS). This project only imports `x/crypto/bcrypt`, not the affected `ssh` subpackage, but the module-level bump was applied since the advisories are scoped to the whole module. The new `x/crypto` requires `go >= 1.25.0` (was `1.22`) — `iso27001-gin/Dockerfile` base image, root `.github/workflows/ci.yml` `go-version`, and README badges/prereqs updated to match. `go build`, `go vet`, and `go test ./...` (21/21) pass
- `iso27001-fastapi/app/api/v1/health.py`: `/health/ready` — an unauthenticated endpoint — no longer echoes raw exception text (`str(exc)`) back to the caller on a DB failure (CodeQL: information exposure through an exception). The detail is now logged server-side via the structured logger (`app.core.telemetry.logger`, correlation-ID tagged) and the response body only reports `{"status": "error"}`. `pytest` (50/50) passes
- `iso27001-gin/go.mod`: `github.com/jackc/pgx/v5` 5.4.3→5.10.0, closing 5 GitHub-flagged advisories (2 Critical memory-safety, SQL injection via protocol message size overflow, SQL injection via dollar-quoted-string placeholder confusion, panic when `PgConn` is busy/closed). Bumped via the direct dependency, `gorm.io/driver/postgres` 1.5.7→1.6.2 (which pulled `gorm.io/gorm` 1.25.10→1.31.2 transitively), rather than pinning the indirect `pgx` requirement directly. No further `go` language-version bump needed (stays at `1.25.0`). `go build`, `go vet`, and `go test ./tests/... -race` (22/22) pass
